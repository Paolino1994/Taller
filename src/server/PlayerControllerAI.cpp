...
PlayerControllerAI::PlayerControllerAI(PlayerModel *model, PlayerView *view) : PlayerController(model, view)
{
	this->log = Log::get_instance();
	this->getModel()->setIsControlledByHuman(false);
}

PlayerControllerAI::PlayerControllerAI(PlayerController *other) : PlayerController(other)
{
	this->log = Log::get_instance();
	this->getModel()->setIsControlledByHuman(false);
}
...
void PlayerControllerAI::_update(double dt, int x_limit, int y_limit, int ball_x, int ball_y)
{
	...
	{ // FORMA CABEZA DE EVITAR QUE SE MUEVAN LOS JUGADORES ANTES DE QUE EMPIECE EL PARTIDO
		...
		{ // Si la pelota esta en la zona entonces la va a buscar
			...
			{ // Si esta muy cerca de x deja de correr porque si no queda girando en el lugar y es feo
		...
		else if (abs(direction_x_initial) >= max_distance_x && abs(direction_x_initial) < max_distance_x + margin)
		{ // Si el jugador esta en limite de su zona entonces me quedo parado
			direction_x_goto = 0;
		}
		else
		{ // El jugador siempre intenta acercarse a la pelota en todo momento
			direction_x_goto = (direction_x_ball >= 0) ? 1 : -1;
		}
		if (teamHasBall && abs(direction_x_ball) < 100)
		{ // Si se acerca mucho a la pelota y la tiene su equipo entonces se queda quieto
			direction_x_goto = 0;
		}
		...
		if (teamHasBall && abs(direction_y_ball) < 100)
		{ // Si se acerca mucho a la pelota y la tiene su equipo entonces se queda quieto
			direction_y_goto = 0;
		}
